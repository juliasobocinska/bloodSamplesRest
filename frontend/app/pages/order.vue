...
const submitOrder = async () => {
  ...
  if (selectedTests.value.length > 5) {
    errorMessage.value = 'Możesz wybrać maksymalnie 5 badań.'
    return
  }

  // --- DYNAMICZNY WYBÓR METODY I ADRESU URL ---
  const isEdit = !!orderId.value
  const url = isEdit ? `http://localhost:5000/orders/${orderId.value}` : 'http://localhost:5000/orders'
